...
_AUTHOR_SIGNATURE = "PRUTHVI_TARS_BUILT_AND_OWNED_BY_PRUTHVI_2026"
_WATERMARK_ID = "PRUTHVI_ORIGINAL_BUILD"


class C:
    # TARS monolith palette — matte gunmetal black, brushed-steel greys,
    # amber/white readouts (Endurance mission console + TARS's own status bars)
    BG        = "#020202"
    PANEL     = "#0a0a0a"
    PANEL2    = "#0e0e0e"
    BORDER    = "#262626"
...
class HudCanvas(QWidget):
    ...
    def paintEvent(self, _):
        """
        TARS — a matte-black articulated monolith made of stacked rectangular
        segments (per the Endurance mission robots), rather than a circular
        Iron-Man style HUD. Segments shift and separate while speaking, like
        TARS's body reconfiguring mid-sentence; they lock rigid when muted.
        """
        p = QPainter(self)
        ...
        accent   = qcol(C.MUTED_C) if self.muted else qcol(C.ACC)
        edge_col = qcol(C.PRI, min(255, int(60 + self._halo)))
        body_top = QColor("#1c1c1c")
        body_bot = QColor("#050505")

        # ── the monolith: stacked articulated segments ──────────────────────
        n_seg   = 8
        seg_w   = fw * 0.30 * self._scale
        total_h = fw * 0.60
        seg_h   = total_h / n_seg
        top_y   = cy - total_h / 2
        ...
        for i in range(n_seg):
            phase = self._tick * speed + i * 0.85
            off_x = math.sin(phase) * amp * (0.35 + 0.65 * ((i + 1) % 3) / 2)
            gap   = 3
            y     = top_y + i * seg_h
            rect  = QRectF(cx - seg_w / 2 + off_x, y + gap / 2, seg_w, seg_h - gap)
            ...
            # amber/status accent strip — alternates sides like panel seams
            strip_w = max(3.0, seg_w * 0.045)
            if i % 2 == 0:
                p.fillRect(QRectF(rect.left(), rect.top(), strip_w, rect.height()), accent)
            else:
                p.fillRect(QRectF(rect.right() - strip_w, rect.top(), strip_w, rect.height()), accent)

            # thin engraved centre seam
            p.setPen(QPen(qcol(C.TEXT_DIM, 90), 1))
            p.drawLine(QPointF(rect.left() + 8, rect.center().y()),
                       QPointF(rect.right() - 8, rect.center().y()))

            # corner rivets
            p.setPen(Qt.PenStyle.NoPen)
            p.setBrush(QBrush(qcol(C.BORDER_B, 200)))
            for rx, ry in [(rect.left()+5, rect.top()+4), (rect.right()-5, rect.top()+4),
                           (rect.left()+5, rect.bottom()-4), (rect.right()-5, rect.bottom()-4)]:
                p.drawEllipse(QPointF(rx, ry), 1.4, 1.4)

        # ── side telemetry brackets flanking the monolith ───────────────────
        br_w   = fw * 0.10
        br_gap = seg_w * 0.5 + 14
        bar_a  = min(255, int(70 + self._halo))
        for side in (-1, 1):
            bx = cx + side * br_gap
            p.setPen(QPen(qcol(C.PRI, bar_a), 1.2))
            p.drawLine(QPointF(bx, top_y), QPointF(bx + side * 10, top_y))
            p.drawLine(QPointF(bx, top_y), QPointF(bx, top_y + total_h))
            p.drawLine(QPointF(bx, top_y + total_h), QPointF(bx + side * 10, top_y + total_h))
            n_ticks = 6
            for t in range(n_ticks + 1):
                ty = top_y + total_h * t / n_ticks
                tl = 5 if t % 2 == 0 else 3
                p.drawLine(QPointF(bx, ty), QPointF(bx + side * tl, ty))

        # ── pulse rings emanating outward when speaking/active ──────────────
        for pr in self._pulses:
            a = max(0, int(160 * (1.0 - pr / (fw * 0.74))))
            col = qcol(C.MUTED_C if self.muted else C.PRI, a)
            p.setPen(QPen(col, 1)); p.setBrush(Qt.BrushStyle.NoBrush)
            rr = QRectF(cx - pr, top_y + total_h/2 - pr*0.55, pr * 2, pr * 1.1)
            p.drawRect(rr)

        # scanning sweep line across the whole HUD
        sweep_y = top_y - 40 + ((self._scan / 360.0) * (total_h + 80))
        sa = min(200, int(40 + self._halo))
        p.setPen(QPen(qcol(C.ACC if not self.muted else C.MUTED_C, sa), 1))
        p.drawLine(QPointF(cx - fw*0.42, sweep_y), QPointF(cx + fw*0.42, sweep_y))

        # corner brackets (mission-console framing)
        bl = 24
        bc = qcol(C.PRI, 200)
        ...
        # designation label centred over the monolith
        p.setFont(QFont("Courier New", 12, QFont.Weight.Bold))
        p.setPen(QPen(qcol(C.PRI, min(255, int(90 + self._halo))), 1))
        p.drawText(QRectF(cx - 100, top_y - 34, 200, 22),
                   Qt.AlignmentFlag.AlignCenter, self._assistant_name)

        # particles (loose debris / dust kicked up while speaking)
        ...
        # status text — sits under the monolith like a console readout
        sy = top_y + total_h + 22
        if self.muted:
        ...
        # waveform — amber when speaking, dim steel otherwise
        wy = sy + 30
        N, bw = 36, 8
        wx0 = (W - N * bw) / 2
        for i in range(N):
            if self.muted:
                ...
                cl  = qcol(C.ACC) if hgt > 12 else qcol(C.ACC2, 180)
            else:
...
class CustomizeOverlay(QWidget):
    ...
    def __init__(self, assistant_name="JARVIS", user_name="",
        ...
        self._hex_input.setPlaceholderText("#e8e4da   (custom hex colour)")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        self.setWindowTitle(f"{_display} — TARS UNIT 3")
    ...
    def _build_header(self) -> QWidget:
        ...
        lay.addWidget(_badge("TARS · UNIT 3", C.PRI_DIM))
        lay.addSpacing(8)
        ...
        elif _disp in ("TARS", "T.A.R.S"):
            _sub_text = "Tactical Assistance & Reconnaissance System"
        else:
    ...
    def _apply_name_update(self, name: str, user_name: str, ui_color: str = ""):
        ...
        self.setWindowTitle(f"{display} — TARS UNIT 3")
        self._title_lbl.setText(display)
        if display in ("JARVIS", "J.A.R.V.I.S"):
            self._sub_lbl.setText("Just A Rather Very Intelligent System")
        elif display in ("TARS", "T.A.R.S"):
            self._sub_lbl.setText("Tactical Assistance & Reconnaissance System")
        else:
