...
export async function openWebAction(text: string): Promise<WebAction> {
  const response = await fetch(`${API_BASE_URL}/web-actions/open`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ text }),
  });
  if (!response.ok) throw new Error("Jarvis could not open that web action.");
  return response.json() as Promise<WebAction>;
}
